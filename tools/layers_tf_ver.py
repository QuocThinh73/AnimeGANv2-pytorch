...
def convolution(x, channels, kernel=4, stride=2, pad=0, pad_type='zero', use_bias=True, spectral_normalization=False, scope='conv0'):
    with tf.compat.v1.variable_scope(scope):
        ...
        if spectral_normalization:
            ...
            if use_bias:
                bias = tf.compat.v1.get_variable("bias", [channels], initializer=tf.zeros_initializer())
...
def resblock(x_init, channels, use_bias=True, scope='resblock0'):
    with tf.compat.v1.variable_scope(scope):
        with tf.compat.v1.variable_scope('res1'):
            x = convolution(x_init, channels, kernel=3, stride=1, pad=1, pad_type='reflect', use_bias=use_bias)
            x = instance_norm(x)
            x = relu(x)

        with tf.compat.v1.variable_scope('res2'):
            x = convolution(x, channels, kernel=3, stride=1, pad=1, pad_type='reflect', use_bias=use_bias)
            x = instance_norm(x)
