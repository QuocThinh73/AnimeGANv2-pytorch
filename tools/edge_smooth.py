...
def parse_args():
    ...
    parser.add_argument('--device', type=str, default='auto',
                        choices=['auto', 'cpu', 'cuda'], help='Device to run blur on')
...
def process_image_torch(bgr_img: np.ndarray, mask: np.ndarray, device: torch.device) -> np.ndarray:
    ...
    out = out.squeeze(0).permute(1, 2, 0).clamp(0, 255).to(
        'cpu').numpy().astype(np.uint8)  # (H, W, 3)
    return out


def process_image_opencv_fast(bgr_img: np.ndarray, mask: np.ndarray) -> np.ndarray:
    blurred = cv2.GaussianBlur(
        bgr_img, (5, 5), 0, borderType=cv2.BORDER_REFLECT_101)
    out = bgr_img.copy()
