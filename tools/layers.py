...
class BatchNorm(nn.Module):
    def __init__(self, num_features, momentum=0.9, eps=1e-5, affine=True):
        super().__init__()
        self.bn = nn.BatchNorm2d(num_features, eps=eps, momentum=(1.0 - momentum), affine=affine)
...
class Convolution(nn.Module):
    def __init__(
        ...
        conv = nn.Conv2d(in_channels, out_channels, kernel_size=kernel, stride=stride, padding=0, bias=use_bias)
        self.conv = spectral_norm(conv) if spectral_normalization else conv

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = _apply_pad(x, self.pads, mode="reflect" if self.pad_type == "reflect" else "constant")
        return self.conv(x)
...
class Deconvolution(nn.Module):
    def __init__(
        ...
        deconv = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=kernel, stride=stride, padding=padding, bias=use_bias)
        self.deconv = spectral_norm(deconv) if spectral_normalization else deconv
...
class ResBlock(nn.Module):
    def __init__(self, channels, use_bias=True):
        ...
        self.conv1 = nn.Conv2d(channels, channels, kernel_size=3, stride=1, padding=0, bias=use_bias)
        self.in1 = nn.InstanceNorm2d(channels, affine=True, eps=1e-5)
        self.pad2 = nn.ReflectionPad2d(1)
        self.conv2 = nn.Conv2d(channels, channels, kernel_size=3, stride=1, padding=0, bias=use_bias)
...
def discriminator_loss(loss_func, real: torch.Tensor, gray: torch.Tensor, fake: torch.Tensor, real_blur: torch.Tensor) -> torch.Tensor:
    ...
    if loss_func in {"wgan-gp", "wgan-lp"}:
        ...
        gray_loss = F.binary_cross_entropy_with_logits(gray, torch.ones_like(gray))
        fake_loss = F.binary_cross_entropy_with_logits(fake, torch.ones_like(fake))
        real_blur_loss = F.binary_cross_entropy_with_logits(real_blur, torch.ones_like(real_blur))
...
def content_loss(vgg, real: torch.Tensor, fake: torch.Tensor) -> torch.Tensor:
    ...
    real_feature_map = getattr(real_feature_map, "conv4_3_no_activation", real_feature_map)

    fake_feature_map = vgg(fake)
    fake_feature_map = getattr(fake_feature_map, "conv4_3_no_activation", fake_feature_map)
...
def content_style_loss(vgg, real: torch.Tensor, anime: torch.Tensor, fake: torch.Tensor):
    ...
    anime_feature_map = getattr(anime_feature_map, "conv4_3_no_activation", anime_feature_map)
