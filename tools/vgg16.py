...
class VGG16(nn.Module):
    def __init__(self, vgg16_npy_path='vgg16_weight/vgg16.npy', include_fc=False):
        ...
        self.register_buffer("vgg_mean", torch.tensor(
            VGG_MEAN, dtype=torch.float32).view(1, 3, 1, 1))

    def _get_convolutional_parameters(self, name, x):
        w_np = self.data_dict[name][0]
        b_np = self.data_dict[name][1]
        w = torch.from_numpy(w_np).permute(
            3, 2, 0, 1).contiguous().to(dtype=x.dtype, device=x.device)
    ...
    def _fc_layer(self, x, name):
        w_np = self.data_dict[name][0]
        b_np = self.data_dict[name][1]
        w = torch.from_numpy(w_np).t().contiguous().to(
    ...
    def forward(self, rgb, return_dict=True):
        ...
        conv4_3_no_activation = self._convolution_no_activation(
            conv4_2, "conv4_3")
